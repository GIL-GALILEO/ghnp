...
def page_search(d):
    ...
    # basic query params (could be facet limits...)
    if d.get('lccn', None):
    ...
    if begin_date or end_date:
        q.append('+date_date:[%s TO %s]' % (begin_date, end_date))

    # ocrs = ['ocr_%s' % l for l in settings.SOLR_LANGUAGES]
    # ocr_lang = 'ocr'

    if d.get('ortext', None):
        q.append('+((' + query_join(solr_escape(d['ortext']).split(' '), "ocr_vector"))
        q.append('))')
    if d.get('andtext', None):
    ...
    facet_params = {
        'facet': 'true',
        'facet_field': ['lccn', 'county', 'city', 'newspaper_type', 'region', 'year'],
        'facet_mincount': 1
    }
